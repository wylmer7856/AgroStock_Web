...
// ⚠️ ADVERTENCIA: La tabla 'alertas_stock' NO EXISTE en la base de datos actual
// Este modelo está deshabilitado hasta que se agregue la tabla a la BD
...
export class AlertasModel {
  ...
  public async ListarAlertas(): Promise<AlertaData[]> {
    // Tabla alertas_stock no existe en la BD
    console.warn("⚠️ La tabla alertas_stock no existe en la base de datos. Retornando array vacío.");
    return [];
    /* Código original comentado hasta que se agregue la tabla:
    try {
  ...
  private async CrearAlerta(producto: ProductoData): Promise<void> {
    // Tabla alertas_stock no existe en la BD - función deshabilitada
    console.warn(`⚠️ No se puede crear alerta para producto ${producto.id_producto}: la tabla alertas_stock no existe en la BD.`);
    return;
    /* Código original comentado:
    try {
      let mensaje = '';
  ...
  public async GenerarAlertasAutomaticas(): Promise<{ alertasCreadas: number; mensaje: string }> {
    // Tabla alertas_stock no existe en la BD
    console.warn("⚠️ La tabla alertas_stock no existe en la base de datos. No se pueden generar alertas automáticas.");
    return {
      alertasCreadas: 0,
      mensaje: "La funcionalidad de alertas está deshabilitada porque la tabla alertas_stock no existe en la base de datos."
    };
    /* Código original comentado:
    try {
  ...
  public async MarcarAlertaComoResuelta(id_alerta: number): Promise<{ success: boolean; message: string }> {
    ...
    return {
      success: false,
      message: "La tabla alertas_stock no existe en la base de datos."
    };
    /* Código original comentado:
    try {
