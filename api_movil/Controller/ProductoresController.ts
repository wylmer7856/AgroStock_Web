  ...
  try {
    ...
    try {
      ...
      if (dbError.message && dbError.message.includes("doesn't exist")) {
        ctx.response.status = 200; // 200 para que no sea un error crítico
        ctx.response.body = {
          success: false,
          error: "TABLA_NO_EXISTE",
          message: "La tabla de productores no está disponible. Los datos se guardarán localmente en tu navegador."
        };
      } else {
