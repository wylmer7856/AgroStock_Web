...
export class AdminController {
  ...
  // 📌 Verificar estado de mantenimiento (público, sin autenticación)
  static async VerificarMantenimiento(ctx: Context) {
    try {
      // Verificar si existe la tabla configuracion_sistema
      const tableExists = await conexion.query(`
        SELECT COUNT(*) as count 
      ...
      // Obtener solo el estado de mantenimiento
      const mantenimientoRows = await conexion.query(`
        SELECT valor 
        FROM configuracion_sistema
      ...
      if (mantenimientoRows.length > 0) {
        const valor = mantenimientoRows[0].valor;
        mantenimiento = valor === 'true' || valor === '1' || valor === 1 || valor === true;
      }
      ...
      ctx.response.status = 200; // En caso de error, asumir que no hay mantenimiento
      ctx.response.body = { 
        success: true, 
        mantenimiento: false,
        message: "Error verificando mantenimiento, asumiendo sistema operativo"
      };
    }
  }
