...
const ProductorMensajesPage: React.FC = React.memo(() => {
  ...
  const { data: mensajesRecibidos, isLoading: loadingRecibidos, error: errorRecibidos, refetch: refetchRecibidos } = useQuery({
    ...
    // NO refrescar automáticamente - solo cuando el usuario envía un mensaje
  });
  ...
  const sendMessageMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      // Invalidar queries pero SIN refetch automático - solo marcar como stale
      queryClient.invalidateQueries({ 
        queryKey: ['mensajes', 'productor'],
        refetchType: 'none' // No refetch automático
      });
      queryClient.invalidateQueries({ 
        queryKey: ['mensajes', 'productor', 'recibidos'],
        refetchType: 'none'
      });
      queryClient.invalidateQueries({ 
        queryKey: ['mensajes', 'productor', 'enviados'],
        refetchType: 'none'
      });
      queryClient.invalidateQueries({ 
      ...
      // Solo refetch manual si hay una conversación seleccionada
